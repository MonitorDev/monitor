...
String realPath = "F:\\apache-tomcat-6.0.36\\apache-tomcat-6.0.36\\webapps\\itask7beta\\pl\\ueditor\\server\\upload\\"+filePath;
//判断路径是否存在，不存在则创建
...
request.setCharacterEncoding("UTF-8");
    ...
    while(fii.hasNext()){
        ...
        try{
            if(!fis.isFormField() && fis.getName().length()>0){            	
				...
				String attachPath = "baiduAttach/" + SDF.format(new Date());
       			ResUtils r = new ResUtilsImpl();
				resId = r.switchFilePath("0000000000010067",url,attachPath, "", "", loginUser,"UD"+loginUser,fileName);
				title=fileName;
            }else{
            	request.setCharacterEncoding("UTF-8");
    ...
    response.getWriter().print("{'url':'attachUpload.sp?act=download&id="+resId+"','title':'"+title+"','state':'"+state+"'}");
