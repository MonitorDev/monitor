...
AmCharts.AmGraph = AmCharts.Class({
		...
		for (O = K; O <= S; O++) {
			...
			if (sa) {
				...
				case "smoothedLine":
					...
					break;
				case "step":
					isNaN(V) ? s || (t = NaN, this.drawLineGraph(q, v, ea, fa), q = [], v = [], ea = [], fa = []) : (F.isNegative = V < ha ? !0 : !1, p ? (isNaN(u) || (q.push(u), v.push(C - ta)), v.push(C - ta), q.push(D), v.push(C + Na), q.push(D), !Ja || (isNaN(N) || isNaN(P)) || (ea.push(w), fa.push(N - ta), ea.push(P), fa.push(N - ta), ea.push(P), fa.push(N + Na))) : (isNaN(t) || (v.push(t), q.push(D - ta)), q.push(D - ta), v.push(C), q.push(D + Na), v.push(C), !Ja || (isNaN(N) || isNaN(P)) || (ea.push(P - ta), fa.push(y), ea.push(P - ta), fa.push(N), ea.push(P + Na), fa.push(N))), u = D, t = C, w = P, y = N, Z = D, ca = C, oa = D, pa = C);
					break;
				case "column":
					ja = qa;
					...
					if (!isNaN(V)) {
						...
						if (!(V < rb && ia < rb || V > sb && ia > sb)) if (p) {
							...
							if (M < l && 0 < J && (la = new AmCharts.Cuboid(e, aa, J, z - a.d3x, H - a.d3y, wa, Ma, R, ja, W, G, n, p), "bottom" != I)) if (I = f.reversed ? "left" : "right", 0 > V) I = f.reversed ? "right" : "left";
							...
							if (L < k + T * z && 0 < J) if (la = new AmCharts.Cuboid(e, J, aa, z - a.d3x, H - a.d3y, wa, Ma, R, ja, this.lineAlpha, G, n, p), 0 > V && "middle" != I) I = "bottom";
	...
	drawLineGraph: function(a, b, c, d) {
		...
		if (1 < a.length) {
			...
			if (0 < f || 0 < v) if (l = a.join(";").split(";"), m = b.join(";").split(";"), "serial" == e.chartType && (0 < c.length ? (c.reverse(), d.reverse(), l = a.concat(c), m = b.concat(d)) : e.rotate ? (m.push(m[m.length - 1]), l.push(u), m.push(m[0]), l.push(u), m.push(m[0]), l.push(l[0])) : (l.push(l[l.length - 1]), m.push(u), l.push(l[0]), m.push(u), l.push(a[0]), m.push(m[0]))), 0 < f && (a = AmCharts.polygon(g, l, m, n, f, 0, 0, 0, this.gradientRotation), h.push(a)), q || void 0 !== s) isNaN(v) && (v = f), q || (q = s), g = AmCharts.polygon(g, l, m, q, v, 0, 0, 0, this.gradientRotation), k.push(g), k.click(function() {
				e.handleGraphClick()
			});
			e.applyMask(k, h)
		}
